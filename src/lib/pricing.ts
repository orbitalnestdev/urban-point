/**
 * Precios. Fuente única para vitrina y checkout. [C-03, C-06]
 *
 * Todos los importes viven en CENTAVOS y como enteros. El storefront mostraba
 * `precio_promocional` mientras createCheckout cobraba siempre `precio`, así
 * que el cliente veía $8.000 y Mercado Pago le cobraba $10.000. Además la home
 * y la página del nodo ignoraban la promo y el catálogo la aplicaba: el mismo
 * producto valía distinto según la página.
 */
...
export function tienePromocion(producto: ProductoPrecio): boolean {
	...
	const lista = aCentavosEnteros(producto?.precio);
...
export function precioDeVentaCentavos(producto: ProductoPrecio): number {
	return tienePromocion(producto)
		...
		: aCentavosEnteros(producto.precio);
}
...
export function precioListaCentavos(producto: ProductoPrecio): number | null {
	return tienePromocion(producto) ? aCentavosEnteros(producto.precio) : null;
}
